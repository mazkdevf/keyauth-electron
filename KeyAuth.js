...
class KeyAuth {
    ...
    constructor(name, ownerId, secret, version) {
        ...
        sessionStorage.setItem('appCredentials', JSON.stringify({
            name: this.name,
    ...
    Initialize = () => new Promise(async (resolve) => {
        ...
        sessionStorage.setItem('appInfo', JSON.stringify(Json.appinfo));
    ...
    Register = (user, password, license) => new Promise(async (resolve) => {
        ...
        if (Json.success) {
            sessionStorage.setItem('userData', JSON.stringify(Json.info))
...
class Encryption {
    ...
    static decrypt(message, key, iv) {
        try {
            ...
            Misc.error('Invalid Application Information. Long text is secret short text is ownerid. Name is supposed to be app name not username')
            alert('Invalid Application Information. Long text is secret short text is ownerid. Name is supposed to be app name not username')
        }
    };
